...
<!-- -->
    ...
    ## [1] "1st approach time: 19.78 vs 2nd approach time: 12"
...
``` r
leaflet(geostations) %>% addTiles() %>% addAwesomeMarkers(lng = ~longUpd, lat= ~latUpd, popup = ~as.character(CN), label = ~as.character(STANAME))
```
