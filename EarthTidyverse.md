...
<!-- -->
    ...
    ## [1] "1st approach time: 19.49 vs 2nd approach time: 12"
...
We calculate the *`percentage of Nas`* for the duration of the previous dataset using [https://cran.r-project.org/web/packages/purrr/index.html](purrr) a *`core tidyverse`* package So, for each year in the interval *`[Year, Yearlead1]`* we get the NAs from the *`MissingdaysYear`* dataset
